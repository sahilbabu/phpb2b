...
class Areas extends PbModel {
	...
	function getLevelAreas()
	{
		...
		if (!empty($result)) {
			foreach ($result as $key=>$val) {
				$return[$key]['id'] = $key;
				$return[$key]['name'] = pb_lang_split($val);
				...
				if (!empty($data)) {
					foreach ($data as $level_val) {
						if($level_val['parent_id'] == $key){
							...
							$return[$key]['sub'][$level_val['id']]['name'] = pb_lang_split($level_val['name']);
		...
		$r = array();
 		...
 		foreach ($ids as $key=>$val) {
 			if($link){
 				...
 				$r[] = "<a href='".smarty_function_the_url(array("module"=>"search",  "action"=>"lists", "do"=>$do, "areaid"=>$the_id))."' rel='special_link'>".$datas[$val]."</a>";
 			}else{
 				$r[] = $datas[$val];
 			}
 		}
