	...
	function index()
	{
		...
		if (isset($_GET['action'])) {
			...
			if ($action == "search") {
				...
				$result = $this->dict->findAll("Dict.*,dp.name AS typename", array("LEFT JOIN {$this->dict->table_prefix}dicttypes dp ON dp.id=Dict.dicttype_id"), $conditions, "Dict.id DESC", $pos, $limit);
				if (!empty($result)) {
					setvar("items", $result);
