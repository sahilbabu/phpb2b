...
//					$conditions[] = 'n.if_focus>0';
	...
	if(list($key, $item) = each($smarty->blockvars[$param_count])) {
		$repeat = true;
		$item['rownum'] = $key;
		$url = smarty_function_the_url(array("module"=>"news", "id"=>$item['id']));
		$item['url'] = $url;
		$item['title'] = pb_lang_split($item['title']);
		$item['fulltitle'] = pb_lang_split($item['fulltitle']);
		...
		$item['typename'] = $item['type_id']?pb_lang_split($_PB_CACHE['newstype'][$item['type_id']]):L("undefined_sort", "tpl");
