...
if(file_exists(PHPB2B_ROOT. 'languages'.DS.strtolower($accept_language).DS."default.dot")) 
...
switch($step)
{
	...
	if($conn){
		...
		if(file_exists($schema_common_path. "mysql.sql"))
		{
			...
			$_languages = serialize($show_languages);
			$db->query("REPLACE INTO {$tb_prefix}settings (variable, valued) VALUES ('languages', '$_languages')");
			//:~
